...
CONTAINS
  ...
  FUNCTION new_mpf_vector_from_mpfs(mpf_array) RESULT(vec)
    ...
    DO i = 1, vec%n
      vec%mantissas(i) = mpi_shift_bits_left(mpf_array(i)%mantissa, mpf_array(i)%exponent - vec%exponent)
    END DO
  ...
  SUBROUTINE mpf_trsm(mat_a, mat_b, uplo)
    ...
    DO j = 1, mat_b%n
      ! 1. Extract j-th column of B into a temporary mpf_vector
      b_col = new_mpf_vector_from_mpfs( &
          (/ (new_mpf_from_mpi_exp(mat_b%mantissas(i,j), mat_b%exponent), i=1,mat_b%m) /) )

      ! 2. Solve the system for this column using the existing trsv routine
      CALL mpf_trsv(mat_a, b_col, uplo)

      ! 3. Store the resulting solution vector into our temporary array of hpfs
      DO i = 1, b_col%n
          solution_mpfs(i,j) = new_mpf_from_mpi_exp(b_col%mantissas(i), b_col%exponent)
      END DO
    END DO

    ! 4. Reconstruct the final matrix from the array of solution columns
