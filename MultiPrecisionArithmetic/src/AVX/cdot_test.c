...
extern int64_t dot_scalar(const int64_t *a, const int64_t *b, size_t n);
extern int64_t dot_avx512(const int64_t *a, const int64_t *b, size_t n);
...
int main()
{
    ...
    int64_t *a, *b;
    printf("\n Benchmarking C DOT (int64) — scalar vs AVX-512\n");
    printf(" ============================================================================\n");
    ...
    for (int t = 0; t < ntests; t++) {
        ...
        b = aligned_alloc(64, N * sizeof(int64_t));

        srand(0);
        for (size_t i = 0; i < N; i++) {
            a[i] = (rand() % 2000000L) - 1000000L;
            b[i] = (rand() % 2000000L) - 1000000L;
        }
        ...
        for (int rep = 0; rep < nrep; rep++) {
            double t0 = now();
            ref = dot_scalar(a, b, N);
            double t1 = now();
            tsum_s += (t1 - t0);

            t0 = now();
            res = dot_avx512(a, b, N);
            t1 = now();
            tsum_v += (t1 - t0);
        }
