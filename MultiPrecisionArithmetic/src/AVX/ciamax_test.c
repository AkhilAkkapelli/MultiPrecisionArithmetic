...
extern size_t iamax_scalar(const int64_t *x, size_t n);
extern size_t iamax_avx512(const int64_t *x, size_t n);
...
int main(void)
{
    ...
    printf("\n Benchmarking C IAMAX (index of max abs value, int64)\n");
    printf(" ======================================================================================================================\n");
    ...
    for (int t = 0; t < ntests; t++) {
        ...
        for (int rep = 0; rep < nrep; rep++) {
            double t0 = now();
            idx_s = iamax_scalar(x, N);
            double t1 = now();
            ts_s[rep] = t1 - t0;

            t0 = now();
            idx_v = iamax_avx512(x, N);
            double t2 = now();
            ts_v[rep] = t2 - t0;
